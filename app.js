...
window.addEventListener("mousedown", () => {
    if (weaponVerif == 1) {
        if (
            player1.position.x > weapon.position.x + weapon.image.width &&
            player1.position.x + player1.width > weapon.position.x
        ) {
            // TODO
            // Idle animation with sword
            weaponVerif = 0;
            weapon.position.y = 200;
        }
    }

    if (player1.canAttack) {
        if (player1.weapon.type === "sword") {
            player1.framesElapsed = 0;
            player1.framesCurrent = 0;
            // TODO
            // Frames + sword attack animation
            player1.framesMax = 3;
